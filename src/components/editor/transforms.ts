...
> = {
    ...
    [ACTION_THREE_COLUMNS]: (editor) =>
        insertColumnGroup(editor, {columns: 1, select: true}),
...
export const insertBlock = (editor: PlateEditor, type: string) => {
    editor.tf.withoutNormalizing(() => {
        ...
        if (type in insertBlockMap) {
            ...
            const isEmptyBlock =
                Array.isArray(block[0].children) &&
                block[0].children.length === 1 &&
                'text' in block[0].children[0] &&
                block[0].children[0].text === '';

            editor.tf.insertNodes(
                editor.api.create.block({type}),
                {
                    at: isEmptyBlock ? block[1] : PathApi.next(block[1]),
                    select: true,
                }
            );
        }
...
> = {
    ...
    [ACTION_THREE_COLUMNS]: (editor) => toggleColumnGroup(editor, {columns: 1}),
};
